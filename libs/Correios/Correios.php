...
class Correios {
	...
	static public function frete($dados) {
		...
		$tipos = array('sedex' => '40010', 'sedex_a_cobrar' => '40045', 'sedex_10' => '40215', 'sedex_hoje' => '40290', 'pac' => '41106');
		...
		$soap = @new SoapClient($endpoint, array('trace' => true, 'exceptions' => true, 'compression' => SOAP_COMPRESSION_ACCEPT | SOAP_COMPRESSION_GZIP, 'connection_timeout' => 1000));
		...
		foreach ($resultado as $consulta) { $consulta = (array)$consulta;
			$dados = array('codigo' => $consulta['Codigo'], 'valor' => (float) str_replace(',', '.', $consulta['Valor']), 'prazo' => (int) str_replace(',', '.', $consulta['PrazoEntrega']), 'mao_propria' => (float) str_replace(',', '.', $consulta['ValorMaoPropria']), 'aviso_recebimento' => (float) str_replace(',', '.', $consulta['ValorAvisoRecebimento']), 'valor_declarado' => (float) str_replace(',', '.', $consulta['ValorValorDeclarado']), 'entrega_domiciliar' => ($consulta['EntregaDomiciliar'] === 'S' ? true : false), 'entrega_sabado' => ($consulta['EntregaSabado'] === 'S' ? true : false), 'erro' => array('codigo' => (real)$consulta['Erro'], 'mensagem' => $consulta['MsgErro']), );
		}
		return $dados;
	}
	...
	static public function cep($endereco) {
		include (ROOT . 'libs/phpQuery/phpQuery-onefile.php');
		$html = self::simpleCurl('http://m.correios.com.br/movel/buscaCepConfirma.do', array('cepEntrada' => $endereco, 'tipoCep' => '', 'cepTemp' => '', 'metodo' => 'buscarCep', ));
